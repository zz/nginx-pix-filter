...
typedef struct {
    ...
    ngx_http_complex_value_t    *pbcv;

    size_t                       buffer_size;
} ngx_http_pix_filter_conf_t;
...
typedef struct {
    ...
    ngx_uint_t                   type;
    ngx_uint_t                   force;
} ngx_http_pix_filter_ctx_t;


static ngx_int_t ngx_http_image_send(ngx_http_request_t *r,
    ngx_http_pix_filter_ctx_t *ctx, ngx_chain_t *in);
...
static ngx_uint_t ngx_http_pix_filter_get_value(ngx_http_request_t *r,
    ngx_http_complex_value_t *cv, ngx_uint_t v);
static ngx_uint_t ngx_http_pix_filter_value(ngx_str_t *value);


static void *ngx_http_pix_filter_create_conf(ngx_conf_t *cf);
static char *ngx_http_pix_filter_merge_conf(ngx_conf_t *cf, void *parent,
    void *child);
static char *ngx_http_pix_filter(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_pix_filter_pixelate(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_http_pix_filter_jpeg_quality(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_http_pix_filter_webp_quality(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_http_pix_filter_sharpen(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ngx_int_t ngx_http_pix_filter_init(ngx_conf_t *cf);


static ngx_command_t  ngx_http_pix_filter_commands[] = {

    { ngx_string("pix_filter"),
    ...
    { ngx_string("pix_filter_jpeg_quality"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_pix_filter_jpeg_quality,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("pix_filter_pixelate"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_pix_filter_pixelate,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("pix_filter_webp_quality"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_pix_filter_webp_quality,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("pix_filter_sharpen"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_pix_filter_sharpen,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("pix_filter_transparency"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_pix_filter_conf_t, transparency),
      NULL },

    { ngx_string("pix_filter_interlace"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_pix_filter_conf_t, interlace),
      NULL },

    { ngx_string("pix_filter_buffer"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_pix_filter_conf_t, buffer_size),
      NULL },
...
static ngx_http_module_t  ngx_http_pix_filter_module_ctx = {
    NULL,                                  /* preconfiguration */
    ngx_http_pix_filter_init,            /* postconfiguration */
    ...
    ngx_http_pix_filter_create_conf,     /* create location configuration */
    ngx_http_pix_filter_merge_conf       /* merge location configuration */
};


ngx_module_t  ngx_http_pix_filter_module = {
    NGX_MODULE_V1,
    &ngx_http_pix_filter_module_ctx,     /* module context */
    ngx_http_pix_filter_commands,        /* module directives */
...
ngx_http_image_header_filter(ngx_http_request_t *r)
{
    off_t                          len;
    ngx_http_pix_filter_ctx_t   *ctx;
    ngx_http_pix_filter_conf_t  *conf;

    if (r->headers_out.status == NGX_HTTP_NOT_MODIFIED) {
        return ngx_http_next_header_filter(r);
    }

    ctx = ngx_http_get_module_ctx(r, ngx_http_pix_filter_module);

    if (ctx) {
        ngx_http_set_ctx(r, NULL, ngx_http_pix_filter_module);
        return ngx_http_next_header_filter(r);
    }

    conf = ngx_http_get_module_loc_conf(r, ngx_http_pix_filter_module);
    ...
    ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_pix_filter_ctx_t));
    if (ctx == NULL) {
        return NGX_ERROR;
    }

    ngx_http_set_ctx(r, ctx, ngx_http_pix_filter_module);
...
static ngx_int_t
ngx_http_image_send(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx,
    ngx_chain_t *in)
{
    ngx_int_t  rc;
...
ngx_http_image_process(ngx_http_request_t *r)
{
    ...
    ctx->max_width = ngx_http_pix_filter_get_value(r, conf->wcv, conf->width);
    if (ctx->max_width == 0) {
        return NULL;
    }

    ctx->max_height = ngx_http_pix_filter_get_value(r, conf->hcv,
                                                      conf->height);
    if (ctx->max_height == 0) {
        return NULL;
    }
...
static ngx_buf_t *
ngx_http_image_json(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx)
{
    size_t      len;
    ngx_buf_t  *b;
...
static ngx_buf_t *
ngx_http_image_asis(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx)
{
    ngx_buf_t  *b;
...
static ngx_int_t
ngx_http_image_size(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx)
{
    u_char      *p, *last;
    size_t       len, app;
...
static ngx_buf_t *
ngx_http_image_resize(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx)
{
    int                            sx, sy, dx, dy, ox, oy, ax, ay, size,
                                   colors, palette, transparent, sharpen, pixelate,
                                   red, green, blue, t;
    u_char                        *out;
    ngx_buf_t                     *b;
    ngx_uint_t                     resize;
...
transparent:
    ...
    sharpen = ngx_http_pix_filter_get_value(r, conf->shcv, conf->sharpen);
    if (sharpen > 0) {
    ...
    pixelate = ngx_http_pix_filter_get_value(r, conf->pbcv, conf->pixelate);
    if (pixelate>0) { 
    	gdImagePixelate(dst, pixelate, GD_PIXELATE_UPPERLEFT);
    }
...
static gdImagePtr
ngx_http_image_source(ngx_http_request_t *r, ngx_http_pix_filter_ctx_t *ctx)
{
    char        *failed;
    gdImagePtr   img;
    ...
    int *size)
{
    ...
    case NGX_HTTP_IMAGE_JPEG:
        ...
        q = ngx_http_pix_filter_get_value(r, conf->jqcv, conf->jpeg_quality);
        if (q <= 0) {
            return NULL;
        }
    ...
    case NGX_HTTP_IMAGE_WEBP:
        ...
        q = ngx_http_pix_filter_get_value(r, conf->wqcv, conf->webp_quality);
        if (q <= 0) {
            return NULL;
        }
...
static ngx_uint_t
ngx_http_pix_filter_get_value(ngx_http_request_t *r,
    ngx_http_complex_value_t *cv, ngx_uint_t v)
{
    ngx_str_t  val;
    ...
    return ngx_http_pix_filter_value(&val);
}


static ngx_uint_t
ngx_http_pix_filter_value(ngx_str_t *value)
{
    ngx_int_t  n;
...
static void *
ngx_http_pix_filter_create_conf(ngx_conf_t *cf)
{
    ngx_http_pix_filter_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_pix_filter_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_pix_filter_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_pix_filter_conf_t *prev = parent;
    ngx_http_pix_filter_conf_t *conf = child;
...
static char *
ngx_http_pix_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_pix_filter_conf_t *imcf = conf;
    ...
    if (cf->args->nelts == 2) {
        ...
        if (ngx_strcmp(value[i].data, "rotate") == 0) {
            ...
            if (cv.lengths == NULL) {
                n = ngx_http_pix_filter_value(&value[i]);
...
static char *
ngx_http_pix_filter_jpeg_quality(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_pix_filter_conf_t *imcf = conf;
    ...
    if (cv.lengths == NULL) {
        n = ngx_http_pix_filter_value(&value[1]);
...
static char *
ngx_http_pix_filter_pixelate(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_pix_filter_conf_t *imcf = conf;
...
static char *
ngx_http_pix_filter_webp_quality(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_pix_filter_conf_t *imcf = conf;
...
static char *
ngx_http_pix_filter_sharpen(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_pix_filter_conf_t *imcf = conf;
...
static ngx_int_t
ngx_http_pix_filter_init(ngx_conf_t *cf)
{
